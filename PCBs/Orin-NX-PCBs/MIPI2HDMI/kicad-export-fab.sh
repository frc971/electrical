#!/bin/bash
#run with ./kicad-export-fab.sh {directory name}
#ex ./kicad-export-fab.sh PowerDistribution


set -x
kicad_dir="/Applications/KiCad/KiCad.app/contents/MacOS"
...
function export_gerber () {
    ...
    $kicad_dir/kicad-cli pcb export gerber --output "${GERBER_DIR}/${fname}-${layer_fname}.gbr" --layers ${layer} ${pcb}
}
...
$kicad_dir/kicad-cli sch export pdf --output "${SCH_DIR}/${fname}-sch-$timestamp.pdf" ${sch}
...
$kicad_dir/kicad-cli pcb export drill --output ${GERBER_DIR}/ ${pcb}

#generate PCB drill map
$kicad_dir/kicad-cli pcb export drill --output ${TEMP_DIR}/ --generate-map --map-format svg ${pcb}

#generate PCB Fab Drawing
$kicad_dir/kicad-cli pcb export pdf --output ${GERBER_DIR}/${fname}-fab.pdf --layers ${pcb_fab_layers} --include-border-title --black-and-white ${pcb}
...
$kicad_dir/kicad-cli pcb export pdf --output ${FAB_DIR}/${fname}-assy.pdf --layers ${pcb_assembly_layers} --include-border-title --black-and-white ${pcb}

#generate PnP file
$kicad_dir/kicad-cli pcb export pos --output ${FAB_DIR}/${fname}-top-pos.txt --side front ${pcb}
$kicad_dir/kicad-cli pcb export pos --output ${FAB_DIR}/${fname}-bot-pos.txt --side back ${pcb}

#create zip file
#zip -r ${SCH_DIR}/${fname}-src-${timestamp}.zip ${src_files//,/ } 

#generate step file
$kicad_dir/kicad-cli pcb export step --output ${SCH_DIR}/${fname}.step --subst-models ${pcb}
pushd ${SCH_DIR}
